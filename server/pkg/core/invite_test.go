...
func TestRejectInvite(t *testing.T) {
	...
	err = inviteContainer.RejectInvite(context.Background(), user.ID, invite.ID)
	assert.NoError(t, err)
}

func TestErrorRejectNotPendingInvite(t *testing.T) {
	db := core.CreateTempDB()
	defer db.Close()
...
func TestErrorRejectInviteFromAnotherUser(t *testing.T) {
	db := core.CreateTempDB()
	defer db.Close()
	...
	err = inviteContainer.RejectInvite(context.Background(), anotherUser.ID, invite.ID)
	assert.Equal(t, err, core.ErrInviteDoesNotExists)
}
