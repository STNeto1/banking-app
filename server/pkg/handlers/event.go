...
type createTransferEventRequest struct {
...
//	@Consumes	json
//	@Produce	json
//	@Param		body	body		createTransferEventRequest	true	"Event params"
//	@Success	201		{object}	GenericSuccessResponse
//	@Failure	400		{object}	GenericErrorResponse
//	@Failure	500		{object}	GenericErrorResponse
//	@Router		/events/transfer [post]
//
//	@Security	ApiKeyAuth
func (c *Container) CreateTransferHandler(ctx echo.Context) error {
	req := new(createTransferEventRequest)
	if err := ctx.Bind(req); err != nil {
		return ctx.JSON(http.StatusBadRequest, GenericErrorResponse{
			Message: "Invalid request",
		})
	...
	err = c.eventContainer.CreateTransferEvent(ctx.Request().Context(), usr.ID, req.ToUserID, decimal.NewFromFloat(req.Amount))
	if err != nil {
		if err == core.ErrInternalError {
			return ctx.JSON(http.StatusInternalServerError, GenericErrorResponse{
				Message: "Internal error",
